...
var spawn = require('child_process').spawn
    ...
    , busyboxARMPath = path.resolve(__dirname, '../bin/busybox-armv6l')
    , busyboxX86Path = path.resolve(__dirname, '../bin/busybox-i686')
    , logger = require('./logger')
...
ADB.prototype.isEmulator = function (cb) {
  this.getConnectedEmulators(function (error, emulators) {
    cb(emulators.length > 0);
  })
};
...
ADB.prototype.runAdbShellCommand = function (command, cb) {
  this.shell(command, function (err, stdout) {
    if (err) return cb(err);
...
ADB.prototype.setWifi = function (on, cb) {
  this.isEmulator(function (isEmulator) {
    var cmd;
    if (isEmulator) {
      cmd = 'svc wifi ' + (on === 1 ? 'enable' : 'disable')
    } else {
      cmd = 'am start -n io.appium.settings/.Settings -e wifi ' + (on === 1 ? 'on' : 'off');
    }
    this.shell(cmd, cb);
  }.bind(this));
};
...
ADB.prototype.setData = function (on, cb) {
  this.isEmulator(function (isEmulator) {
    var cmd;
    if (isEmulator) {
      cmd = 'svc data ' + (on === 1 ? 'enable' : 'disable')
    } else {
      cmd = 'am start -n io.appium.settings/.Settings -e data ' + (on === 1 ? 'on' : 'off');
    }
    this.shell(cmd, cb);
  }.bind(this));
};
...
ADB.prototype.setWifiAndData = function (opts, cb) {
  ...
  if (typeof opts.wifi !== 'undefined') {
    this.setWifi(opts.wifi, callback);
  }

  if (typeof opts.data !== 'undefined') {
    this.setData(opts.data, callback);
  }
};
...
ADB.prototype.setOwnerRecursively = function (file, user, group, cb) {
  var cmd = 'su -c chown -R ' + user + ':' + group + ' ' + file;
  this.shell(cmd, function (err) {
    if (err) cb("Set owner error");
    cb();
  });
...
ADB.prototype.getABI = function (cb) {
  var cmd = 'getprop | grep ro.product.cpu.abi]';
  this.shell(cmd, function (err, stdout) {
    if (err) return cb(err);
    cb(null, stdout.indexOf('x86') ? 'x86' : 'arm');
  });
};

ADB.prototype.busybox = function (cmd, cb) {
  var busyBoxPath = "/data/local/",
      busyBoxBin = "busybox",
      executeCommand = function () {
        this.shell(busyBoxPath + busyBoxBin + ' ' + cmd, function (err, stdout) {
          if (err) return cb(err);
          cb(null, stdout);
        });
      }.bind(this);
  this.ls(busyBoxPath, function (err, files) {
    if (err) return cb(err);
    if (files.indexOf(busyBoxBin) == -1) {
      this.installBusybox(busyBoxPath + busyBoxBin, function (err) {
        if (err) return cb(err);
        executeCommand();
      }.bind(this));
    } else {
...
ADB.prototype.chmod = function (file, mod, cb) {
  var cmd = 'su -c chmod ' + mod + ' ' + file;
  this.shell(cmd, function (err) {
    if (err) cb("chmod error");
    cb();
  });
};

ADB.prototype.installBusybox = function (path, cb) {
  this.getABI(function (err, abi) {
    if (err) return cb(err);
    this.push(abi === 'arm' ? busyboxARMPath : busyboxX86Path, path, function (err) {
      if (err) return cb(err);
      this.chmod(path, '0755', function (err) {
        if (err) return cb(err);
        cb();
      });
    }.bind(this));
